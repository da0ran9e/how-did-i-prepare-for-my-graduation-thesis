...
#include "node/communication/routing/bCastRouting/BCastRouting.h"

Define_Module(BCastRouting);

void BCastRouting::startup()
{
...
void BCastRouting::sendTopologySetupPacket()
{
	BCastRoutingPacket *setupPkt =
	    new BCastRoutingPacket("Multipath rings routing setup packet", NETWORK_LAYER_PACKET);
	setupPkt->setBCastRoutingPacketKind(MPRINGS_TOPOLOGY_SETUP_PACKET);
...
void BCastRouting::sendControlMessage(BCastRoutingControlDef kind)
{
	BCastRoutingControlMessage *ctrlMsg =
	    new BCastRoutingControlMessage("Multipath routing control message",NETWORK_CONTROL_MESSAGE);
	ctrlMsg->setBCastRoutingControlMessageKind(kind);
	ctrlMsg->setLevel(currentLevel);
	ctrlMsg->setSinkID(currentSinkID);
	toApplicationLayer(ctrlMsg);
}

void BCastRouting::timerFiredCallback(int index)
{
...
void BCastRouting::processBufferedPacket()
{
...
void BCastRouting::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	string dst(destination);

	BCastRoutingPacket *netPacket =
	    new BCastRoutingPacket("Multipath rings routing data packet", NETWORK_LAYER_PACKET);
	netPacket->setBCastRoutingPacketKind(MPRINGS_DATA_PACKET);
...
void BCastRouting::fromMacLayer(cPacket * pkt, int macAddress, double rssi, double lqi)
{
	BCastRoutingPacket *netPacket = dynamic_cast <BCastRoutingPacket*>(pkt);
	if (!netPacket)
		return;

	switch (netPacket->getBCastRoutingPacketKind()) {
			...
			if (dst.compare(BROADCAST_NETWORK_ADDRESS) == 0 ||
				...
				if (senderLevel == currentLevel + 1) {
					if (self == sinkID) {
						...
						BCastRoutingPacket *dupPacket = netPacket->dup();
