...
void Netformation::fromNetworkLayer(ApplicationPacket* rcvPacket, const char* source, double rssi, double lqi) {
...
void Netformation::timerFiredCallback(int index) {
...
void Netformation::handleRadioControlMessage(RadioControlMessage* radioMsg) {
...
void Netformation::finishSpecific() {
	...
	for (int i = 0; i < numNodes; i++) {
		Netformation* appModule = dynamic_cast<Netformation*>(topo->getNode(i)->getModule()->getSubmodule("Application"));
		if (appModule) {
