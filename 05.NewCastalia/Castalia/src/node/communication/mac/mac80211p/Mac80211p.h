...
#ifndef _Mac80211p_H_
#define _Mac80211p_H_

#include "VirtualMac.h"
#include "Mac80211pPacket_m.h"
...
enum Mac80211pTimers {
	...
	WAKEUP_SILENT = 8,	// this timer has to be last as Mac80211p will 
						// schedule multiple timers for silent wakeup 
						// if more than one secondary schedule is present
};

struct Mac80211pSchedule {
	simtime_t offset;
	int ID;
	int SN;
};

class Mac80211p: public VirtualMac {
 private:
	...
	/*--- Mac80211p activation timeout variable ---*/
	simtime_t activationTimeout;	//time untill MAC_CHECK_TA message arrival

	/*--- Mac80211p packet pointers (sometimes packet is created not immediately before sending) ---*/
	Mac80211pPacket *syncPacket;
	Mac80211pPacket *rtsPacket;
	Mac80211pPacket *ctsPacket;
	Mac80211pPacket *ackPacket;

	/*--- Mac80211p Schedule table (list of effective schedules) ---*/
	vector<Mac80211pSchedule> scheduleTable;
 ...
 protected:
	...
	void seMac80211pState(int newState, const char *descr = NULL);
	void createPrimarySchedule();
	void scheduleSyncPacket(simtime_t when);
	void sendDataPacket();
	void processMacPacket(Mac80211pPacket *);
	void carrierIsClear();
	void carrierIsBusy();
