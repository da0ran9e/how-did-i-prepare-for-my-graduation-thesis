...
void TraCIScenarioManager::init_traci()
{
...

    {
        ...
        EV << "Send to SUMO CMDID: " << "0xdb:CMD_SUBSCRIBE_SIM_VARIABLE" << ": " << buf1.str() << endl;
...

    {
        ...
        EV << "Send to SUMO CMDID: " << "0xd4:CMD_SUBSCRIBE_VEHICLE_VARIABLE" << ": " << beginTime <<":"<< endTime <<":"<< objectId <<":"<< variableNumber <<":"<< variable1 << endl;
        processSubcriptionResult(buf);
        ASSERT(buf.eof());
    }
...
void TraCIScenarioManager::executeOneTimestep()
{
    ...
    if (isConnected()) {
        ...
        EV << "Send to SUMO CMDID: " << "0x02:CMD_SIMSTEP2"  << ": " << targetTime.str() << endl;
...
void TraCIScenarioManager::subscribeToVehicleVariables(std::string vehicleId)
{
    ...
    EV << "Send to SUMO CMDID: " << "0xd4:CMD_SUBSCRIBE_VEHICLE_VARIABLE"  << ": " << buf1.str() << endl;
    processSubcriptionResult(buf);
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::unsubscribeFromVehicleVariables(std::string vehicleId)
{
    ...
    EV << "Send to SUMO CMDID: " << "0xd4:CMD_SUBSCRIBE_VEHICLE_VARIABLE" << ": " << beginTime <<":"<< endTime <<":"<< objectId <<":"<< variableNumber << endl;
    ASSERT(buf.eof());
}
void TraCIScenarioManager::subscribeToTrafficLightVariables(std::string tlId)
{
    ...
    EV << "Send to SUMO CMDID: " << "0xd2:CMD_SUBSCRIBE_TL_VARIABLE"  << ": " << beginTime.str() <<":" << endTime.str() <<":" << objectId <<":" << variableNumber <<":" << variable1<<":" << variable2 <<":" << variable3 <<":" << variable4<<":" << endl;
    processSubcriptionResult(buf);
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::unsubscribeFromTrafficLightVariables(std::string tlId)
{
    ...
    EV << "Send to SUMO CMDID: " << "0xd2:CMD_SUBSCRIBE_TL_VARIABLE" << beginTime << endTime << objectId << variableNumber << endl;
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::processSubcriptionResult(TraCIBuffer& buf)
{
    ...
    if (commandId_resp == RESPONSE_SUBSCRIBE_VEHICLE_VARIABLE) {
        EV << "Response from SUMO: ID: 0xe4:RESPONSE_SUBSCRIBE_VEHICLE_VARIABLE" << buf.str() << endl;
        processVehicleSubscription(objectId_resp, buf);
    }
    else if (commandId_resp == RESPONSE_SUBSCRIBE_SIM_VARIABLE) {
        EV << "Response from SUMO: ID: 0xeb:RESPONSE_SUBSCRIBE_SIM_VARIABLE" << buf.str() << endl;
        processSimSubscription(objectId_resp, buf);
    }
    else if (commandId_resp == RESPONSE_SUBSCRIBE_TL_VARIABLE) {
        EV << "Response from SUMO: ID: 0xe2:RESPONSE_SUBSCRIBE_TL_VARIABLE" << buf.str() << endl;
