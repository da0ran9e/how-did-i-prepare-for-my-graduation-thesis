...
{
    int vID = 0 + this->myId;
    Coord curPos = this->curPosition;
    Coord curSpeed = this->curSpeed;
    std::string vCoor = std::to_string(curPos.x) + "; " + std::to_string(curPos.y) + "; " + std::to_string(curPos.z);
    std::string vSpeed = std::to_string(curSpeed.x) + "; " + std::to_string(curSpeed.y) + "; " + std::to_string(curSpeed.z);
    int vRout = this->mySCH;

    std::string info = "{\n\t\"ID\" : \"" + std::to_string(vID) +"\""
                    + ", \n\t\"coor\" : \"" + vCoor +"\""
                    + ", \n\t\"speed\" : \"" + vSpeed +"\""
                    + ", \n\t\"rout\" : \"" + std::to_string(vRout) +"\"\n}";

    BaseFrame1609_4 *netPacket = check_and_cast <BaseFrame1609_4*>(msg);
    netPacket->setContent(info.c_str());

    checkAndTrackPacket(netPacket);
    BaseApplLayer::sendDown(netPacket);
}
